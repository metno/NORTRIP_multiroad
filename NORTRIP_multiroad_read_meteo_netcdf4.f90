    ...
    if (invert_dim_flag(x_index).or.invert_dim_flag(y_index)) then
        ...
        if (invert_dim_flag(x_index)) then
            write(unit_logfile,'(A)') ' Inverting X dimmension'

            do i=1,dim_length_nc(x_index)
                var1d_nc(:,i)=var1d_nc_temp(:,dim_length_nc(x_index)+1-i)
        ...
        if (invert_dim_flag(y_index)) then
            write(unit_logfile,'(A)') ' Inverting Y dimmension'
            do j=1,dim_length_nc(y_index)
                var1d_nc(:,j)=var1d_nc_temp(:,dim_length_nc(y_index)+1-j)
    ...
    !Not correct but it might be now

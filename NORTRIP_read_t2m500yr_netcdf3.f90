    ...
    subroutine NORTRIP_read_t2m500yr_netcdf3
	...
	write(unit_logfile,'(A)') 'Reading meteorological data (NORTRIP_read_t2m500yr_netcdf)'
	write(unit_logfile,'(A)') '================================================================'
    ...
    if (.not.exists) then
        write(unit_logfile,'(A,A)') ' ERROR: Meteo temperature netcdf file does not exist: ', trim(pathfilename_nc2)
    ...
    write(unit_logfile,'(2A)') ' Opening netcdf meteo temperature file: ',trim(pathfilename_nc2)
    status_nc2 = NF_OPEN (pathfilename_nc2, NF_NOWRITE, id_nc2)
    if (status_nc2 .NE. NF_NOERR) write(unit_logfile,'(A,I)') 'ERROR opening netcdf file: ',status_nc2

    !Find out the x,y and time dimmensions of the file
    status_nc2 = NF_INQ_DIMID (id_nc2,dim_name_nc2(x_index2),dim_id_nc2(x_index2))
    status_nc2 = NF_INQ_DIM (id_nc2,dim_id_nc2(x_index2),dimname_temp,dim_length_nc2(x_index2))
    status_nc2 = NF_INQ_DIMID (id_nc2,dim_name_nc2(y_index2),dim_id_nc2(y_index2))
    status_nc2 = NF_INQ_DIM (id_nc2,dim_id_nc2(y_index2),dimname_temp,dim_length_nc2(y_index2))
    status_nc2 = NF_INQ_DIMID (id_nc2,dim_name_nc2(time_index2),dim_id_nc2(time_index2))
    status_nc2 = NF_INQ_DIM (id_nc2,dim_id_nc2(time_index2),dimname_temp,dim_length_nc2(time_index2))
    ...
    do i=1,num_dims_nc2
        status_nc2 = NF_INQ_VARID (id_nc2, trim(dim_name_nc2(i)), var_id_nc2(i))
        status_nc2 = NF_GET_VARA_REAL (id_nc2, var_id_nc2(i), dim_start_nc2(i), dim_length_nc2(i), var1d_nc2(i,:))
        !status_nc2 = NF_GET_VAR (id_nc2, var_id_nc2(i), var1d_nc2(i,1:dim_length_nc2(i)), start=(/dim_start_nc2(i)/), count=(/dim_length_nc2(i)/))
    ...
    do i=1,num_var_nc2
        ...
        status_nc2 = NF_INQ_VARID (id_nc2, trim(var_name_nc2(i)), var_id_nc2(i))
        ...
        if (status_nc2.eq.NF_NOERR) then
    ...
    end subroutine NORTRIP_read_t2m500yr_netcdf3